...
test_that("get_etalab_raw() returns NULL when no URLs", {
  with_mocked_bindings(
...
test_that("get_etalab_raw() works online with httptest2 mocks", {
  skip_if_not_installed("httptest2")

  httptest2::with_mock_dir("get_etalab_raw", {
    # Single commune, multiple layers
    res1 <- get_etalab_raw("72187", c("parcelles", "lieux_dits"), verbose = FALSE)
